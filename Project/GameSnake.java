...
public class GameSnake {
    ...
    void go() {
        ...
        while (!gameOver) {//áåçïåðåðâíèé öèêë
            snake.move();
            if (food.isEaten()) {
                food.next();
            }
            canvasPanel.repaint();
            try {
                Thread.sleep(DELAY);//òîðìîç³ííÿ ðóõó çì³éêè
            } catch (InterruptedException e) { e.printStackTrace(); }//äîçâîëÿý ðóõàòèñÿ
        }
    }
    ...
    class Snake {
        ...
        public Snake(int x, int y, int length, int direction) {
            for (int i = 0; i < length; i++) {//ðîáèìî çì³éêó ç ïîéíò³â
        ...
        boolean isInsideSnake(int x, int y) {
            for (Point point : snake) {
                if ((point.getX() == x) && (point.getY() == y)) {//ñìåðòü çì³éêè
                    return true;
                }
            }
            return false;
        ...
        void move() {
            ...
            int y = snake.get(0).getY();//ãîëîâà óïðàâëÿº âñ³ì
            if (direction == LEFT) { x--; }
            if (direction == RIGHT) { x++; }
            if (direction == UP) { y--; }
            if (direction == DOWN) { y++; }
            if (x > PROGRAM_WIDTH - 1) { x = 0; }//ðîáèìî ùîá çà ìåæû íå âèõîäèëà, à ïîâåðòàëàñÿ íà 0 êîîðäèíàò
        ...
        void setDirection(int direction) {
            if ((direction >= LEFT) && (direction <= DOWN)) {
                if (Math.abs(this.direction - direction) != 2) {//íå ÿ
                    this.direction = direction;
                }
            }
        }
 
        void paint(Graphics g) {//ìåòîä ìàëþâàííÿ ³íòåðôåéñó
            for (Point point : snake) {
                point.paint(g);
            }
        }
    ...
    class Food extends Point {
        ...
        public Food() {
            super(-1, -1);//ïîëîæåííÿ ïîçà ìåæàìè ïåðøî¿ ¿æ³
